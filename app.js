...
function computeMarginRequirement(positions, scenario, marketState) {
  ...
  const positionsByType = {
    call: positions.filter(position => position.type === 'call' && position.qty !== 0),
    put: positions.filter(position => position.type === 'put' && position.qty !== 0),
  };

  function nakedRequirement(position) {
    ...
    return perContractRequirement * shortContracts;
  }

  function spreadAdjustedRequirement(type) {
    const typePositions = positionsByType[type];
    const longPool = typePositions
      .filter(position => position.qty > 0)
      .map(position => ({ strike: position.strike, qty: position.qty }));
    const shortPositions = typePositions
      .filter(position => position.qty < 0)
      .map(position => ({ ...position, remainingQty: Math.abs(position.qty) }));

    if (type === 'call') {
      longPool.sort((a, b) => a.strike - b.strike);
      shortPositions.sort((a, b) => a.strike - b.strike);
    } else {
      longPool.sort((a, b) => b.strike - a.strike);
      shortPositions.sort((a, b) => b.strike - a.strike);
    }

    let requirement = 0;

    shortPositions.forEach(shortPosition => {
      const nakedPerContract = nakedRequirement({ ...shortPosition, qty: -1 });

      for (const longPosition of longPool) {
        if (shortPosition.remainingQty <= 0) break;
        const isProtectiveLong = type === 'call'
          ? longPosition.strike > shortPosition.strike
          : longPosition.strike < shortPosition.strike;
        if (!isProtectiveLong || longPosition.qty <= 0) continue;

        const offsetQty = Math.min(shortPosition.remainingQty, longPosition.qty);
        const spreadWidth = Math.abs(longPosition.strike - shortPosition.strike) * scenario.contractSize;
        requirement += Math.min(nakedPerContract, spreadWidth) * offsetQty;
        shortPosition.remainingQty -= offsetQty;
        longPosition.qty -= offsetQty;
      }

      if (shortPosition.remainingQty > 0) {
        requirement += nakedPerContract * shortPosition.remainingQty;
      }
    });
  ...
  return spreadAdjustedRequirement('call') + spreadAdjustedRequirement('put');
}
    ...
    <div class="card stack">
      ...
      <div class="grid two">
        <div class="card stack">
          ...
          <div>Time to expiry also falls after each accepted trade. In this simulator, each accepted trade is treated as one market step, so option time value changes as the session progresses even though the platform is not using real clock time.</div>
          <div>Short options consume simplified margin. Basic same-scenario call and put spreads receive margin relief, while uncovered shorts require higher margin. A trade will be rejected if it would leave negative buying power or negative equity.</div>
        </div>
        <div class="card stack">
      ...
      <div class="grid two">
        <div class="card stack">
          ...
          <div>Trade count is based on submitted order events, not on the number of lots. A 3-lot order counts as one trade event.</div>
      ...
      <div class="note">
        ...
        This is a teaching simulator with a simplified margin model. The saved session data, audit hash, and final submission fingerprint are local audit references for this browser session rather than secure proof against tampering.
      </div>
      <div class="actions">
    ...
    <div class="grid two">
      <div class="card stack">
        ...
        <div class="note">
          ...
          Choose the lot size before trading. Each option contract controls ${scenario.contractSize} units of the underlying. Buys must leave non-negative equity. Short option positions consume margin, with basic relief for same-scenario spreads. A multi-lot order still counts as one trade event. Trades are rejected if post-trade buying power would be negative. The market path updates after each accepted trade.
        </div>
        <div class="actions">
...
function renderSummary() {
  ...
  <div class="card stack">
    ...
    <div class="actions">
      ...
      <button class="secondary" onclick="downloadCurrentSessionTradeLog()" ${tradeCount ? '' : 'disabled'}>Download current trade log</button>
  ...
  document.getElementById('submit').innerHTML = `
    <div class="card stack">
      ...
      <div class="note">
        ...
        Final confirmation stores a snapshot of the selected four sessions, their scores, and an audit fingerprint. This is not server-side invigilation or tamper-proof locking, but it gives markers a fixed local record of what was submitted from this browser.
      </div>
      <div class="table-wrap">
        <table>
      ...
      <div class="actions">
        ...
        <button class="secondary" onclick="downloadSubmittedTradeLogs()" ${(state.finalSubmitted && selectedSessions.length === 4) ? '' : 'disabled'}>Download submitted trade logs</button>
...
function beginScenario(code) {
  if (state.current?.activity?.length && !state.current.saved) {
    const shouldDiscard = window.confirm('You have an unsaved session with recorded trades. Starting a new scenario will replace it. Do you want to continue?');
...
function tradeOption(optionId, side) {
  ...
  const thetaEffect = signedQty * option.theta * scenario.contractSize;
  ...
  current.auditLog.unshift({
    ...
    thetaEffect: Number(thetaEffect.toFixed(4)),
...
function buildTradeLogCsv(sessions, exportType) {
  const header = [
    'export_type',
    'candidate_name',
    'candidate_number',
  ...
  const rows = sessions.flatMap((session, index) => {
    const base = [
      ...
      exportType === 'submitted_sessions' ? (state.finalSubmissionRecord?.submittedAt || '') : '',
      exportType === 'submitted_sessions' ? (state.finalSubmissionRecord?.fingerprint || '') : '',
      index + 1,
      session.sessionId || 'Current session',
      session.scenarioCode || '',
      session.scorecard ? formatNum(session.scorecard.total, 1) : '',
      session.requirements ? (session.requirements.disciplineSatisfied && session.requirements.instrumentCoverageSatisfied ? 'Yes' : 'No') : '',
      session.auditHash || '',
      session.portfolio ? session.portfolio.pnl : '',
      session.portfolio ? session.portfolio.delta : '',
      session.portfolio ? session.portfolio.gamma : '',
      session.portfolio ? session.portfolio.theta : '',
      session.portfolio ? session.portfolio.vega : '',
    ];

    const tradeRows = [...(session.activity || [])].reverse().map((trade, tradeIndex) => {
      const audit = [...(session.auditLog || [])].reverse()[tradeIndex];
      return [
        ...base,
        'trade',
        tradeIndex + 1,
        ...
        audit?.vegaEffect ?? '',
        audit?.spotAfter ?? '',
        audit?.volAfter != null ? Number(audit.volAfter * 100).toFixed(2) : '',
        audit?.cashAfter ?? '',
        '',
        '',
        '',
    ...
    const rejectionRows = [...(session.rejectionLog || [])].reverse().map((rejection, rejectionIndex) => [
      ...base,
      'rejection',
      rejectionIndex + 1,
      rejection.action || '',
      ...
      '',
      '',
      rejection.equityAfter ?? '',
      rejection.marginRequirement ?? '',
      rejection.buyingPower ?? '',
      rejection.reason || '',
    ]);

    return [...tradeRows, ...rejectionRows];
  });
...
function downloadCurrentSessionTradeLog() {
  const { portfolio, scorecard, requirements, scenario, current } = computeCurrentSessionSummary();
  if (!current.activity.length) return;

  const sessionSnapshot = {
    sessionId: current.saved ? 'Current saved session' : 'Current practice session',
    scenarioCode: scenario.code,
    portfolio,
    scorecard,
    requirements,
    ...
    auditLog: clone(current.auditLog),
    rejectionLog: clone(current.rejectionLog),
    auditHash: hashString(JSON.stringify({
      scenarioCode: scenario.code,
      activity: current.activity,
      auditLog: current.auditLog,
      rejectionLog: current.rejectionLog,
  ...
  const content = buildTradeLogCsv([sessionSnapshot], 'current_session');
  downloadFile(`${state.student.number || 'candidate'}_current_trade_log.csv`, content, 'text/csv;charset=utf-8;');
}
window.downloadCurrentSessionTradeLog = downloadCurrentSessionTradeLog;

function downloadSubmittedTradeLogs() {
  if (!state.finalSubmitted) return;

  const submittedIds = state.finalSubmissionRecord?.sessions?.map(s => s.sessionId) || [];
  const submittedSessions = state.savedSessions.filter(s => submittedIds.includes(s.sessionId));
  if (submittedSessions.length !== 4) return;
  const content = buildTradeLogCsv(submittedSessions, 'submitted_sessions');
  downloadFile(`${state.student.number || 'candidate'}_submitted_trade_logs.csv`, content, 'text/csv;charset=utf-8;');
}
window.downloadSubmittedTradeLogs = downloadSubmittedTradeLogs;
