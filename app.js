...
function previewTrade(current, scenario, option, side) {
    ...
    <div class="grid two">
      <div class="card stack">
        ...
        <div class="note">
          ...
          Each trade is for one contract. Buys must leave non-negative equity. Short option positions consume margin, and trades are rejected if post-trade buying power would be negative. The market path updates after each accepted trade.
        </div>
        <div class="actions">
...
function renderSummary() {
  ...
  <div class="card stack">
    ...
    <div class="note">
      ...
      Target fit rewards ending delta inside the published band. Volatility response rewards vega adjustments that move against volatility shocks. Risk control penalises extreme gamma and vega. P&amp;L contributes only after the session shows meaningful activity.
    </div>
...
function tradeOption(optionId, side) {
  ...
  const preview = previewTrade(state.current, scenario, option, side);

  if (preview.portfolio.equity < 0 || preview.buyingPower < 0) {
    const rejectionMessage = `Trade rejected: this ${side} order would leave equity at ${formatMoney(preview.portfolio.equity)} with margin required of ${formatMoney(preview.marginRequirement)}. Reduce size or close risk before adding this trade.`;
