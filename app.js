...
function buildTradeLogText(sessions, title) {
  const lines = [];
  lines.push(`Candidate name: ${state.student.name || 'Not entered'}`);
  lines.push(`Candidate number: ${state.student.number || 'Not entered'}`);
  if (state.finalSubmissionRecord?.submittedAt) lines.push(`Submitted at: ${state.finalSubmissionRecord.submittedAt}`);
  if (state.finalSubmissionRecord?.fingerprint) lines.push(`Submission fingerprint: ${state.finalSubmissionRecord.fingerprint}`);
  lines.push('');
  lines.push(title);

  sessions.forEach((session, index) => {
    lines.push('');
    lines.push(`SESSION ${index + 1}: ${session.sessionId || 'Current session'}`);
    lines.push(`Scenario: ${session.scenarioCode}`);
    if (session.scorecard) lines.push(`Assessment score: ${formatNum(session.scorecard.total, 1)} / 100`);
    if (session.requirements) lines.push(`Requirements met: ${session.requirements.disciplineSatisfied && session.requirements.instrumentCoverageSatisfied ? 'Yes' : 'No'}`);
    lines.push(`Audit hash: ${session.auditHash || '-'}`);
    if (session.portfolio) {
      lines.push(`Final P&L: ${formatMoney(session.portfolio.pnl)}`);
      lines.push(`Final delta: ${formatNum(session.portfolio.delta, 1)}`);
      lines.push(`Final gamma: ${formatNum(session.portfolio.gamma, 2)}`);
      lines.push(`Final theta: ${formatNum(session.portfolio.theta, 2)}`);
      lines.push(`Final vega: ${formatNum(session.portfolio.vega, 1)}`);
    }
    lines.push('Trade log:');

    const orderedTrades = [...(session.activity || [])].reverse();
    const orderedAudit = [...(session.auditLog || [])].reverse();

    orderedTrades.forEach((trade, tradeIndex) => {
      const audit = orderedAudit[tradeIndex];
      lines.push(
        `  ${tradeIndex + 1}. ${trade.action} ${trade.lots || 1} lot${Number(trade.lots || 1) === 1 ? '' : 's'} of ${trade.instrument} at premium ${formatNum(trade.premium, 3)}. ${trade.marketNote || ''}`.trim()
      );
      if (audit) {
        lines.push(
          `     Delta effect ${formatNum(audit.deltaEffect, 2)}, gamma effect ${formatNum(audit.gammaEffect, 4)}, theta effect ${formatNum(audit.thetaEffect, 4)}, vega effect ${formatNum(audit.vegaEffect, 2)}, spot after ${formatNum(audit.spotAfter, 4)}, vol after ${(audit.volAfter * 100).toFixed(2)}%, cash after ${formatMoney(audit.cashAfter)}.`
        );
      }
    });

    if (session.rejectionLog?.length) {
      lines.push('Rejected orders:');
      [...session.rejectionLog].reverse().forEach((rejection, rejectionIndex) => {
        lines.push(
          `  ${rejectionIndex + 1}. ${rejection.action} ${rejection.lots || 1} lot${Number(rejection.lots || 1) === 1 ? '' : 's'} of ${rejection.instrument} rejected. Equity after ${formatMoney(rejection.equityAfter)}, margin required ${formatMoney(rejection.marginRequirement)}, buying power ${formatMoney(rejection.buyingPower)}.`
        );
      });
    }
  });
...
function downloadCurrentSessionTradeLog() {
  ...
  const content = buildTradeLogText([sessionSnapshot], 'This trade log was generated from the current session for practice or review purposes.');
  downloadTextFile(`${state.student.number || 'candidate'}_current_trade_log.txt`, content);
}
...
function downloadSubmittedTradeLogs() {
  ...
  const content = buildTradeLogText(
    submittedSessions,
    'These trade logs correspond to the four sessions locked for final submission.\nStudents can use them to explain trading decisions in the separate written document.'
  );
  downloadTextFile(`${state.student.number || 'candidate'}_submitted_trade_logs.txt`, content);
}
